...
| Date       | Summary |
| ---------- | ------- |
| 2026-06-02 | **Conditional Type 2 cleanups & Advanced Expressions**: Removed the redundant "Condition Type" section and the "Else Branch" section heading. Implemented a "Load More" feature for the Boolean Attributes list in `ConditionalBranchV2Config.tsx`. Added an **Advanced Expression Builder** to Conditional Type 2 (`ConditionalBranchV2Data.advancedConditions`), allowing users to add custom logic condition branches that execute after Boolean checks. Integrated with `ConditionBuilder` drawer and synced via `syncConditionalV2Branches`. |
| 2026-06-02 | **Conditional Type 2 in BranchAddMenu**: Added `CONDITIONAL_V2_OPTION` to `BranchAddMenu.tsx` so users can insert a **Conditional Type 2** block directly from the inline "Add to branch" quick-add popup (the `+` button in branch lines). Imported `defaultNestedConditionalV2Level` and `ToggleRight` icon; option appears between Conditional and Multisplit in the menu for all non-multisplit branch contexts. |
